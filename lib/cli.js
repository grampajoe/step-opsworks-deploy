...
exports = module.exports = function(argv, deploy) {
  ...
  if (!program.region) {
    if (process.env['AWS_DEFAULT_REGION']) {
      program.region = process.env['AWS_DEFAULT_REGION'];
    } else {
  ...
  deploy({
    'access-key-id': program.accessKeyId,
    'secret-access-key': program.secretAccessKey,
    'stack-id': program.stackId,
    'app-id': program.appId,
    'region': program.region,
    'migrate': program.migrate,
